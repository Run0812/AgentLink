...
**ACP Bridge 的作用:**
- Bridge 负责与具体 Agent（Claude Code、Kimi Code 等）的协议转换
- 插件只需实现统一的 ACP 协议
- 可以直接利用已有的 ACP 兼容方案
...
class AcpBridgeAdapter implements AgentAdapter {
  ...
  async connect(): Promise<void> {
    // 1. 启动本地 ACP Bridge 进程
    this.bridgeProcess = spawn(this.config.bridgeCommand, this.config.bridgeArgs, {
      cwd: this.config.workspaceRoot,
      env: { ...process.env, ...this.config.env }
    });
    
    // 2. 建立与 Bridge 的 ACP 连接
    this.acpClient = new ACPClient(this.config.acpServerURL);
  ...
  async sendMessage(input: AgentInput, handlers: StreamHandlers): Promise<void> {
    ...
    // 解析 ACP 响应
    // - type: 'text' | 'thinking' | 'tool_call' | 'file_edit' | 'error'
    // 对于 tool_call，暂停并等待用户确认
  }
}
```
...
project-root/
  ...
  src/
    core/
      types.ts          # 扩展 AgentResponse, ToolCall 等类型
      logger.ts         # ✅ 已有
      errors.ts         # ✅ 已有
    settings/
      settings.ts       # 新增配置项
      settings-tab.ts   # 新增设置 UI
    adapters/
      agent-adapter.ts  # ✅ 已有，扩展工具调用接口
      mock-adapter.ts   # ✅ 已有，增加工具调用模拟
      acp-bridge-adapter.ts    # ❌ 待实现：ACP 协议适配
      embedded-web-adapter.ts  # ❌ 待实现：嵌入 Web UI
    ui/
      chat-view.ts      # ⚠️ 增加工具调用卡片、确认对话框
      tool-call-card.ts # ❌ 新增：工具调用预览组件
      file-diff-view.ts # ❌ 新增：文件修改 diff 预览
    services/
      session-store.ts  # ⚠️ 扩展会话管理
      process-manager.ts # ✅ 已有
      stream-parser.ts   # ✅ 已有
      tool-executor.ts   # ❌ 新增：工具执行服务
  test/
    unit/
    fixtures/
      mock-acp-bridge.js       # ❌ 新增：模拟 ACP Bridge
      mock-embedded-web.html   # ❌ 新增：模拟 Embedded Web Agent
```
